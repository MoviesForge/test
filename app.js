...
function init() {
    ...
    }" style="${UI.fixed_footer ? "position: fixed; " : ""}left: 0; bottom: 0; width: 100%; color: white; z-index: 9999;${
...
function getQueryVariable(e) {
    for (var t, n = window.location.search.substring(1).split("&"), a = 0; a < n.length; a++) if ((t = n[a].split("="))[0] == e) return t[1];
    return !1;
}
function render(e) {
    ...
    window.MODEL.is_search_page
        ? ((window.scroll_status = { event_bound: !1, loading_lock: !1 }), render_search_result_list())
        : e.match(/\/\d+:$/g) || "/" == e.slice(-1)
...
function nav(e) {
    ...
    (n += `<li class="nav-item dropdown"><a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">${o}</a><div class="dropdown-menu" aria-labelledby="navbarDropdown">`),
        ...
        (n += `<li class="nav-item">\n    <a class="nav-link" href="${UI.contact_link}" target="_blank">${UI.nav_link_4}</a>\n  </li>${
            UI.show_logout_button ? '<li class="nav-item"><a class="nav-link" href="/logout">Logout</a></li>' : ""
        }`);
...
function requestListPath(e, t, n, a, i = 3, o = !1) {
    var l = { id: t.id || "", type: "folder", password: t.password || "", page_token: t.page_token || "", page_index: t.page_index || 0 };
    $("#update").show(),
        ...
        (function t() {
            fetch(o ? "/0:fallback" : e, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(l) })
                .then(function (e) {
                    if (!e.ok) throw new Error("Request failed");
                    return e.json();
                })
                .then(function (t) {
                    t && t.error && 401 === t.error.code
                        ? askPassword(e)
                        : t && null === t.data
                        ? (document.getElementById("spinner").remove(), (document.getElementById("list").innerHTML = "<div class='alert alert-danger' role='alert'> Server didn't send any data.</div></div></div>"), $("#update").hide())
                        : t && t.data && (n(t, e, l), $("#update").hide());
                })
                .catch(function (e) {
                    i > 0
                        ? (sleep(2e3), (document.getElementById("update").innerHTML = "<div class='alert alert-info' role='alert'> Retrying...</div></div></div>"), t())
...
function requestSearch(e, t, n = 3) {
    var a = { q: e.q || null, page_token: e.page_token || null, page_index: e.page_index || 0 };
    $("#update").html("<div class='alert alert-info' role='alert'> Connecting...</div></div></div>"),
        (function e(n) {
            fetch(`/${window.current_drive_order}:search`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(a) })
                .then(function (e) {
                    if (!e.ok) throw new Error("Request failed");
                    return e.json();
                })
                .then(function (e) {
                    e && null === e.data && ($("#spinner").remove(), $("#list").html("<div class='alert alert-danger' role='alert'> Server didn't send any data.</div></div></div>"), $("#update").remove()),
                        e && e.data && (t && t(e, a), $("#update").remove());
                })
                .catch(function (t) {
                    n > 0
                        ? (sleep(2e3), $("#update").html("<div class='alert alert-info' role='alert'> Retrying...</div></div></div>"), e(n - 1))
...
function list(e, t = "", n = !1) {
    ...
    function p(e, a, i) {
        console.log(e, a, i),
            ...
            null === e.nextPageToken
                ? ($(window).off("scroll"), (window.scroll_status.event_bound = !1), (window.scroll_status.loading_lock = !1), n ? append_files_to_fallback_list(a, e.data.files) : append_files_to_list(a, e.data.files))
                      ...
                      ($(window).on("scroll", function () {
                          ...
                          if (e + $(this).height() > o - (Os.isMobile ? 130 : 80)) {
                              ...
                              (window.scroll_status.loading_lock = !0),
                                  $(`<div id="spinner" class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`).insertBefore(
                                      "#readme_md"
                                  );
                              let e = $("#list");
                              n
                                  ? (console.log("fallback inside handleSuccessResult"),
                                    requestListPath(a, { id: t, password: i.password, page_token: e.data("nextPageToken"), page_index: e.data("curPageIndex") + 1 }, p, null, 5, t, (n = !0)))
                                  : requestListPath(a, { password: i.password, page_token: e.data("nextPageToken"), page_index: e.data("curPageIndex") + 1 }, p, null);
                          }
                      }),
    ...
    $("#list").html(`<div class="d-flex justify-content-center"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`),
        $("#readme_md").hide().html(""),
        $("#head_md").hide().html(""),
        n ? (console.log("fallback inside list"), requestListPath(e, { id: t, password: c }, p, null, null, (n = !0))) : (console.log("handling this"), requestListPath(e, { password: c }, p, null));
...
function append_files_to_fallback_list(e, t) {
    try {
        ...
        for (i in t) {
            var s = t[i],
                d = "/fallback?id=" + s.id;
            if (((s.modifiedTime = utc2delhi(s.modifiedTime)), "application/vnd.google-apps.folder" == s.mimeType))
                html += `<a href="${d}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${s.name} ${
                    UI.display_time ? '<span class="badge bg-info float-end"> ' + s.modifiedTime + " </span>" : ""
                }</a>`;
            else {
                l = l + Number(s.size);
                ...
                "?a=view",
                    ...
                    (html +=
                        ...
                        (UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' + p + '" id="flexCheckDefault">' : "")),
                    "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${m}|`) >= 0
                        ? (html += video_icon)
                        : "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${m}|`) >= 0
                        ? (html += code_icon)
                        : "|zip|rar|tar|.7z|.gz|".indexOf(`|${m}|`) >= 0
                        ? (html += zip_icon)
                        : "|bmp|jpg|jpeg|png|gif|".indexOf(`|${m}|`) >= 0
                        ? (html += image_icon)
                        : "|m4a|mp3|flac|wav|ogg|".indexOf(`|${m}|`) >= 0
                        ? (html += audio_icon)
                        : "|md|".indexOf(`|${m}|`) >= 0
                        ? (html += markdown_icon)
                        : "|pdf|".indexOf(`|${m}|`) >= 0
                        ? (html += pdf_icon)
                        : (html += file_icon),
                    (html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${d}&a=view">${s.name}</a>${
                        UI.display_download
                            ...
                            : ""
                    }${UI.display_size ? '<span class="badge bg-primary float-end"> ' + s.size + " </span>" : ""}${UI.display_time ? ' <span class="badge bg-info float-end"> ' + s.modifiedTime + " </span>" : ""}</div>`);
            }
        }
        ...
        n.html(("0" == n.data("curPageIndex") ? "" : n.html()) + html),
            a &&
                ((total_size = formatFileSize(l) || "0 Bytes"),
                ...
                0 == total_items
                    ...
                    ? $("#count")
                          .removeClass("d-none")
                          .find(".number")
                          .text(total_items + " item")
                    : $("#count")
                          .removeClass("d-none")
                          .find(".number")
                          .text(total_items + " items"),
                0 == total_files
                    ...
                    ? $("#count")
                          .removeClass("d-none")
                          .find(".totalsize")
                          .text(total_files + " File with Size " + total_size)
                    : $("#count")
                          .removeClass("d-none")
                          .find(".totalsize")
                          .text(total_files + " Files with Size " + total_size));
    } catch (e) {
        console.log(e);
    }
}
function append_files_to_list(e, t) {
    ...
    for (i in t) {
        ...
        if (((d.modifiedTime = utc2delhi(d.modifiedTime)), "application/vnd.google-apps.folder" == d.mimeType))
            html += `<a href="${c}" style="color: ${UI.folder_text_color};" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${d.name} ${
                UI.display_time ? '<span class="badge bg-info float-end"> ' + d.modifiedTime + " </span>" : ""
            }</a>`;
        else {
            r = r + Number(d.size);
            ...
            console.log(f),
                ...
                (html +=
                    '<div class="list-group-item list-group-item-action">' +
                    (UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' + m + '" id="flexCheckDefault">' : "")),
                "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${f}|`) >= 0
                    ? (html += video_icon)
                    : "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${f}|`) >= 0
                    ? (html += code_icon)
                    : "|zip|rar|tar|.7z|.gz|".indexOf(`|${f}|`) >= 0
                    ? (html += zip_icon)
                    : "|bmp|jpg|jpeg|png|gif|".indexOf(`|${f}|`) >= 0
                    ? (html += image_icon)
                    : "|m4a|mp3|flac|wav|ogg|".indexOf(`|${f}|`) >= 0
                    ? (html += audio_icon)
                    : "|md|".indexOf(`|${f}|`) >= 0
                    ? (html += markdown_icon)
                    : "|pdf|".indexOf(`|${f}|`) >= 0
                    ? (html += pdf_icon)
                    : (html += file_icon),
                (html += ` <a class="countitems size_items list-group-item-action" style="text-decoration: none; color: ${UI.css_a_tag_color};" href="${g}">${d.name}</a>${
                    UI.display_download
                        ? `<a href="${m}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>`
                        : ""
                }${UI.display_size ? '<span class="badge bg-primary float-end"> ' + d.size + " </span>" : ""}${UI.display_time ? ' <span class="badge bg-info float-end"> ' + d.modifiedTime + " </span>" : ""}</div>`);
        }
    }
    ...
    n.html(("0" == n.data("curPageIndex") ? "" : n.html()) + html),
        a &&
            ((total_size = formatFileSize(r) || "0 Bytes"),
            (total_items = n.find(".countitems").length),
            (total_files = n.find(".size_items").length),
            0 == total_items
                ? $("#count").removeClass("d-none").find(".number").text("Empty Folder")
...
function render_search_result_list() {
    ...
    $("#content").html(e),
        ...
        $("#readme_md").hide().html(""),
        $("#head_md").hide().html(""),
        requestSearch({ q: window.MODEL.q }, function e(t, n) {
            $("#list").data("nextPageToken", t.nextPageToken).data("curPageIndex", t.curPageIndex),
                $("#spinner").remove(),
                null === t.nextPageToken
                    ? ($(window).off("scroll"), (window.scroll_status.event_bound = !1), (window.scroll_status.loading_lock = !1), append_search_result_to_list(t.data.files))
                      ...
                      !0 !== window.scroll_status.event_bound &&
                          ($(window).on("scroll", function () {
                              ...
                              if (t + $(this).height() > n - (Os.isMobile ? 130 : 80)) {
                                  ...
                                  let t = $("#list");
                                  requestSearch({ q: window.MODEL.q, page_token: t.data("nextPageToken"), page_index: t.data("curPageIndex") + 1 }, e);
                              }
                          }),
                          (window.scroll_status.event_bound = !0))),
                !0 === window.scroll_status.loading_lock && (window.scroll_status.loading_lock = !1);
...
function append_search_result_to_list(e) {
    try {
        ...
        for (i in e) {
            var l = e[i];
            if ((null == l.size && (l.size = ""), (l.modifiedTime = utc2delhi(l.modifiedTime)), "application/vnd.google-apps.folder" == l.mimeType))
                html += `<a style="color: ${UI.folder_text_color};" onclick="onSearchResultItemClick('${
                    l.id
                }', false)" data-bs-toggle="modal" data-bs-target="#SearchModel" class="countitems list-group-item list-group-item-action"> ${folder_icon} ${l.name} ${
                    UI.display_time ? '<span class="badge bg-info float-end"> ' + l.modifiedTime + " </span>" : ""
                }</a>`;
            else {
                ...
                (html += `<div style="color: ${UI.css_a_tag_color};" gd-type="$item['mimeType']}" class="countitems size_items list-group-item list-group-item-action">${
                    UI.allow_selecting_files ? '<input class="form-check-input" style="margin-top: 0.3em;margin-right: 0.5em;" type="checkbox" value="' + s + '" id="flexCheckDefault">' : ""
                }`),
                    "|mp4|webm|avi|mpg|mpeg|mkv|rm|rmvb|mov|wmv|asf|ts|flv|".indexOf(`|${r}|`) >= 0
                        ? (html += video_icon)
                        : "|html|php|css|go|java|js|json|txt|sh|".indexOf(`|${r}|`) >= 0
                        ? (html += code_icon)
                        : "|zip|rar|tar|.7z|.gz|".indexOf(`|${r}|`) >= 0
                        ? (html += zip_icon)
                        : "|bmp|jpg|jpeg|png|gif|".indexOf(`|${r}|`) >= 0
                        ? (html += image_icon)
                        : "|m4a|mp3|flac|wav|ogg|".indexOf(`|${r}|`) >= 0
                        ? (html += audio_icon)
                        : "|md|".indexOf(`|${r}|`) >= 0
                        ? (html += markdown_icon)
                        : "|pdf|".indexOf(`|${r}|`) >= 0
                        ? (html += pdf_icon)
                        : (html += file_icon),
                    (html += ` <span onclick="onSearchResultItemClick('${l.id}', true)" data-bs-toggle="modal" data-bs-target="#SearchModel">${l.name}</span>${
                        UI.display_download
                            ? `<a href="${s}"><svg class="float-end"width="25px" style="margin-left: 8px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"> <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"></path> <path d="M7.646 11.854a.5.5 0 0 0 .708 0l3-3a.5.5 0 0 0-.708-.708L8.5 10.293V1.5a.5.5 0 0 0-1 0v8.793L5.354 8.146a.5.5 0 1 0-.708.708l3 3z"></path> </svg></a>`
                            : ""
                    }<span class="badge float-end csize"> ${UI.display_size ? '<span class="badge bg-primary float-end"> ' + l.size + " </span>" : ""}${
                        UI.display_time ? ' <span class="badge bg-info float-end"> ' + l.modifiedTime + " </span>" : ""
                    }</div>`);
            }
        }
        o && UI.allow_selecting_files && (document.getElementById("select_items").style.display = "block"),
            ...
            n &&
                ((total_size = formatFileSize(a) || "0 Bytes"),
...
function onSearchResultItemClick(e, t) {
    ...
    var o = { id: e };
    fetch(`/${n}:id2path`, { method: "POST", body: JSON.stringify(o), headers: { "Content-Type": "application/x-www-form-urlencoded" } })
        .then(function (e) {
...
async function fallback(e, t) {
    ...
    $("#content").html(`<div class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`),
        fetch("/0:fallback", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: e }) })
            .then(function (e) {
                if (!e.ok) throw new Error("Request failed");
                return e.json();
            })
            .then(function (e) {
                ...
                if ("application/vnd.google-apps.folder" === t) window.location.href = window.location.pathname + "/";
                    ...
                    if (t.includes("video") || o.includes(a)) {
                        ...
                        t.includes("audio") || l.includes(a)
                            ? file_audio(d, c, p, m, g, n)
                            : t.includes("image") || r.includes(a)
                            ? file_image(d, c, p, m, g, n)
                            : t.includes("pdf") || s.includes(a)
                            ? file_pdf(d, c, p, m, g, n)
                            : i.includes(a)
                            ? file_code(d, c, p, m, g, n)
                            : file_others(d, c, p, m, g, n);
                }
            })
            .catch(function (e) {
                var t =
                    `\n          <div class="container"><br>\n          <div class="card text-center">\n            <div class="card-body text-center">\n              <div class="${UI.file_view_alert_class}" id="file_details" role="alert"><b>404.</b> That’s an error. ` +
                    e +
...
async function file(e) {
    ...
    e.split("/").pop();
    $("#content").html(`<div class="d-flex justify-content-center" style="height: 150px"><div class="spinner-border ${UI.loading_spinner_class} m-5" role="status" id="spinner"><span class="sr-only"></span></div></div>`),
        fetch("", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ path: e }) })
            .then(function (e) {
                if (!e.ok) throw new Error("Request failed");
                return e.json();
            })
            .then(function (e) {
                ...
                if ("application/vnd.google-apps.folder" === n) window.location.href = window.location.pathname + "/";
                    ...
                    if (n.includes("video") || o.includes(a)) {
                        ...
                        n.includes("audio") || l.includes(a)
                            ? file_audio(d, c, p, m, g, t)
                            : n.includes("image") || r.includes(a)
                            ? file_image(d, c, p, m, g, t)
                            : n.includes("pdf") || s.includes(a)
                            ? file_pdf(d, c, p, m, g, t)
                            : i.includes(a)
                            ? file_code(d, c, p, m, g, t)
                            : file_others(d, c, p, m, g, t);
                }
            })
            .catch(function (e) {
                var t =
...
function file_video(e, t, n, a, i, o, l, r) {
    ...
    let v;
    UI.disable_player ||
        ("plyr" == player_config.player
            ? ((v = `<video id="player" playsinline controls data-poster="${a}">\n      <source src="${i}" type="video/mp4" />\n      <source src="${i}" type="video/webm" />\n        </video>`),
            ...
            : "videojs" == player_config.player
            ? ((v = `<video id="vplayer" poster="${a}" muted=true class="video-js vjs-default-skin" controls preload="auto" width="100%" height="100%" data-setup='{"fluid": true}' style="--plyr-captions-text-color: #ffffff;--plyr-captions-background: #000000;">\n      <source src="${i}" type="video/mp4" />\n      <source src="${i}" type="video/webm" />\n      <source src="${i}" type="video/avi" />\n    </video>`),
            ...
            : "dplayer" == player_config.player
            ? ((v = '<div id="player-container"></div>'), (player_js = "https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.js"), (player_css = "https://cdn.jsdelivr.net/npm/dplayer/dist/DPlayer.min.css"))
            : "jwplayer" == player_config.player && ((v = '<div id="player"></div>'), (player_js = "https://content.jwplatform.com/libraries/IDzF9Zmk.js"), (player_css = "")));
    var u = `\n    <div class="container text-center"><br>\n      <nav aria-label="breadcrumb">\n        <ol class="breadcrumb">\n          ${p}\n        </ol>\n      </nav>\n      <div class="card text-center">\n        <div class="text-center">\n          <div class="${
        UI.file_view_alert_class
    }" id="file_details" role="alert">${e}<br>${n}</div>${v}</div>\n        </br>\n        ${
    ...
    (h.src = player_js),
        (h.onload = function () {
            if ("plyr" == player_config.player) {
                ...
                new DPlayer({ container: document.getElementById("player-container"), screenshot: !0, video: { url: i, pic: a, thumbnails: a } });
            } else
                "jwplayer" == player_config.player &&
                    jwplayer("player").setup({
                        ...
                        tracks: [{ file: i, kind: "captions", label: "Default", default: !0 }],
                        captions: { color: "#f3f378", fontSize: 14, backgroundOpacity: 50, edgeStyle: "raised" },
                    });
        }),
        document.head.appendChild(h);
    var y = document.createElement("link");
    (y.href = player_css), (y.rel = "stylesheet"), document.head.appendChild(y);
}
function file_audio(e, t, n, a, i, o) {
    ...
    for (var d = "", c = "", p = 0; p < r.length; p++) {
        var m = r[p];
        p == r.length - 1 ? (c += m + "?a=view") : (c += m + "/"), m.length > 15 && (m = (m = decodeURIComponent(m)).substring(0, 10) + "..."), "" == m && (m = "Home"), (d += '<a href="' + c + '" class="breadcrumb-item">' + m + "</a>");
    }
    var g = `\n    <div class="container text-center"><br>\n      <nav aria-label="breadcrumb">\n        <ol class="breadcrumb">\n          ${d}\n        </ol>\n      </nav>\n      <div class="card text-center">\n        <div class="text-center">\n          <div class="${
        UI.file_view_alert_class
    }" id="file_details" role="alert">${e}<br>${n}</div>\n          ${
        UI.disable_player
            ? ""
    ...
    (v.href = "https://vjs.zencdn.net/" + UI.videojs_version + "/video-js.css"), (v.rel = "stylesheet"), document.head.appendChild(v);
}
function file_pdf(e, t, n, a, i, o) {
    ...
    for (var r = "", s = "", d = 0; d < l.length; d++) {
        var c = l[d];
        d == l.length - 1 ? (s += c + "?a=view") : (s += c + "/"), c.length > 15 && (c = (c = decodeURIComponent(c)).substring(0, 10) + "..."), "" == c && (c = "Home"), (r += '<a href="' + s + '" class="breadcrumb-item">' + c + "</a>");
    }
    var p =
        `\n    <div class="container text-center"><br>\n      <nav aria-label="breadcrumb">\n        <ol class="breadcrumb">\n          ${r}\n        </ol>\n      </nav>\n    <div class="card">\n    <div class="card-body text-center">\n    <div class="${
            UI.file_view_alert_class
...
function formatFileSize(e) {
    return (
        e >= 1099511627776
            ? (e = (e / 1099511627776).toFixed(2) + " TB")
            : e >= 1073741824
...
async function copyFile(e) {
    try {
        ...
        if ("" === n) return (t.innerHTML = "<div class='alert alert-danger' role='alert'> Empty ID </div>"), null;
        (document.getElementById("spinner").style.display = "block"), (document.cookie = `root_id=${n}; expires=Thu, 18 Dec 2050 12:00:00 UTC`);
        const a = Math.floor(Date.now() / 1e3),
            i = await fetch("/copy", { method: "POST", cache: "no-cache", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body: `id=${encodeURIComponent(e)}&root_id=${n}&resourcekey=null&time=${a}` });
        if (500 === i.status) t.innerHTML = "<div class='alert alert-danger' role='alert'> Unable to Copy File, Make Sure you've added system@zindex.eu.org to your Destination Folder </div>";
        else if (401 === i.status) t.innerHTML = "<div class='alert alert-danger' role='alert'> Unauthorized </div>";
        else if (i.ok) {
            ...
            if (e && e.name) {
                ...
                document.getElementById(
                    "copyresult"
                ).innerHTML = `<div class="col-12 col-md-12"> <input type="text" id="usercopiedfile" class="form-control" placeholder="Enter Your Folder ID to Copy this File" value="${t}" readonly></div> <div class="col-12 col-md-12"> <a href="${t}" target="_blank" style="margin-top: 5px;" class="btn btn-danger btn-block">Open Copied File</a></div>`;
            } else
        ...
        } else t.innerHTML = "<div class='alert alert-danger' role='alert'> Unable to Copy File </div>";
        document.getElementById("spinner").style.display = "none";
    } catch (e) {
        (document.getElementById("copystatus").innerHTML = "<div class='alert alert-danger' role='alert'> An error occurred " + e + "</div>"), (document.getElementById("spinner").style.display = "none");
    }
}
